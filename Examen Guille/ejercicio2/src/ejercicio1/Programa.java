...
public class Programa {
	...
	public static void main(String[] args) throws FileNotFoundException {
		 ...
		 * La primera vez que se ejecute lanzara una exception EOFException ya que aun no existira el archivo tmp.dat ya que
		...
		if(temporal==null) {
			...
			agenda= new Agenda(temporal);
		}
		
		//creamos una agenda vacia y un objeto Fichero para manejar el mismo
		...
		switch(opcion) {
			...
			case 8:
				for(Contacto c: agenda.getContactos()) {
					c.setSexo(r.nextBoolean());
					if(c.getSexo()==true) {
						c.setHijos(r.nextInt(30));
					}
					else {
						c.setBarba(r.nextBoolean());
					}
				}	
		}//fin switch
		}
